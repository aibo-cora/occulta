...
**Related, not addressed in this pass:** `ContactManager.deleteContact` (`Contact+Manager.swift:450`) also calls `forEachGroup { $0.purgeMember(identifier) }` — the no-key overload, deriving fresh per group. Same shared engine, same optimization would apply, but contact deletion wasn't part of what was scoped or discussed here.
