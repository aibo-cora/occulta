...
**Status: scoped, briefly implemented, then replaced by the much smaller fix above.** Worth stating plainly, not just implied: **Option C, even fully implemented and fully tested, never actually fixed the reported bug.** `VaultManager.addEntry` was never changed to write the new mask — it kept stamping the old ceiling field — and nothing anywhere ever called `setEntryVisible` (no automatic trigger, no UI wiring). So `isEntryVisible`'s mask check would always find nothing migrated, fall back to the legacy ceiling, and the original leak would have persisted for every entry regardless. This wasn't an incomplete feature waiting on one more piece — it was a complete, correctly-built, well-tested subsystem that solved a harder problem (independent multi-depth visibility) than the one that actually needed solving, while leaving the actual bug untouched. Comparable in size to the entire `Group` re-encryption fix (`Group-Reencryption-Multi-Second-Block-On-Visibility-Toggle.md`), not a follow-on tweak — though it turns out to differ from that precedent (and from `GlobalShardConfig`'s per-depth schema change, `Shard-Custody-Not-Cleaned-Up-On-Contact-Deletion.md` item 3) in two important ways: simpler in its data shape, riskier in its scale.
