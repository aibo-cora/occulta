...
**Deferred, not investigated — blocks implementation until resolved:** whether `GlobalShardConfig.trusteeIDs` removal needs the same depth-0-only gating `cleanUpGroupDuressMembership` applies to group duress-depth membership (i.e., is there a decoy/duress-sensitive angle to who's a default trustee, the way group membership at deeper depths can be deliberately different from depth 0) — or whether it's safe to always purge unconditionally, the way `Message.Draft.purge`/`Group.purgeMember` are. Deliberately punted rather than guessed at here; pick this up before writing the `GlobalShardConfig` piece of `purgeCustody(for:)`.
