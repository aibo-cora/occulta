...
**Status:** Gap 1 (contact deletion) implemented. Gap 2 items 1–2 (filter the two live-leak surfaces, fix the merge-not-overwrite save bug) implemented — see "Implementation status" at the bottom. Gap 2 item 3 (`GlobalShardConfig`'s per-depth schema change) is fully scoped, all design decisions made — see item 3 below — but deliberately **not yet implemented**. Items 4–5 (the decoy-custodian store, wiring it into the Vault tab) remain unscoped beyond their original outline. Found while investigating a separate, smaller gap (`Message.Draft` not purged on contact deletion — see `Docs/Features/Message Persistence/FINDINGS.md`, "Investigated: does `reKeyOrPurgeAll`'s `isKnownContact` check let an already-hidden contact's orphaned draft survive?").
...
**Status: fully scoped, decisions made, not yet implemented.**

Confirmed current shape is a single flat `struct Payload: Codable { let trusteeIDs: [String] }` (`GlobalShardConfig+Model.swift:65-67`) — no depth concept, no fixed slot count, no padding. `Group`'s scheme exists specifically so real and filler slots produce *identical ciphertext sizes* and so a diff can't reveal which single depth was just edited (Bug 73) — a naive "one array per depth" that varies in length with content, or that only re-encrypts the touched depth, would recreate both of those exact leaks.

**Shape — not a literal mirror of `Group`.** `Group`'s three-way split (`realMemberSlots` for depth 0, `duressMemberSlots` for depth 1 specifically, `deeperMemberSlots` as an array for depths 2+) is historical schema continuity from Bug 73 — before that fix, depth 1 was a single shared bucket for *all* duress depths, and the split preserved that field for pre-1.9.1 rows while adding independent storage for depths 2+. `GlobalShardConfig` has no such legacy field to stay compatible with. Correct shape: one uniform array, all depths treated identically —
```swift
var trusteeSlots: [[Data]] = []  // trusteeSlots[depth], depth in 0..<depthCount
```

**Slot count: 16, not `Group`'s 32.** No existing precedent to anchor on — `ShamirSecretSharing.split` allows `n <= 255` (a cryptographic field limit, not a realistic bound), and there's no existing UI cap on trustee count anywhere (`VaultShardSetup`/`VaultGlobalTrustees` only enforce a *minimum* of 2). `Group`'s 32 itself isn't an arbitrary convention to match — its own doc comment ties it to `AppLayerConfig.maxVerifierCount`, a constant meaningful to *that* feature. 16 is comfortably above any realistic k-of-n social-recovery setup while halving the mandatory per-edit re-encryption cost (`slotCount × depthCount`) versus reusing 32.

**Where the enforcement lives, and where it doesn't.** Since attempting to select more than 16 has to be blocked in the UI (mirroring `Group.addMember`'s `capacityExceeded` guard), this cap needs enforcing in `VaultGlobalTrustees.swift`'s trustee-toggle logic specifically. **It does not apply to `VaultShardSetup.swift`** — that screen's recipient selection is per-*entry* shard distribution, stored in `VaultEntry.shardDistributionEncrypted`, a completely separate structure that `GlobalShardConfig` only pre-populates suggestions for. `VaultShardSetup` stays governed only by Shamir's own `n <= 255`.

**Read/write shape:** `addTrustee(_:atDepth:)`/`removeTrustee(_:atDepth:)` public pair backed by a private `setTrustees(_:atDepth:)` — matching `Group`'s public/private split in spirit, but living on `ShardCustodyManager`, not on the `GlobalShardConfig` model itself. Unlike `Group` (whose per-depth methods call the ambient `Data.encrypt()`/`.decrypt()` extension directly), `GlobalShardConfig`'s encryption depends on `ShardCustodyManager`'s injected `keyManager` — the model has no access to that on its own.

**No `Group`-style gating needed for the new methods themselves.** `addTrustee`/`removeTrustee` are single-depth, single-identifier edits — same shape as `Group.addMember`/`removeMember(atDepth:)`, which are ungated. There's no equivalent to `cleanUpGroupDuressMembership`'s broader, depth-0-gated hygiene sweep needed here: that exists for `Group` because stale automatic membership can accumulate in duress-depth arrays from earlier states, and trustee selection is always an explicit, deliberate user action with no equivalent stale-cruft scenario. **What does need updating: `purgeCustody(for:)`'s trustee-removal step**, which today filters one flat array — once storage is per-depth, it needs to remove the identifier from *every* depth's slots, mirroring `Group.purgeMember`'s exact reasoning (a deleted contact is invalid everywhere; touching every depth is safe, required, and ungated).

**Consequence for already-shipped code:** the merge-not-overwrite fix in `VaultGlobalTrustees.save()` (items 1–2, shipped in `9e439e3`) exists *because* storage was flat — saving a filtered subset risked wholesale-deleting hidden trustees. Once storage is per-depth, that problem shape disappears entirely: each depth's slot is edited independently, so the picker should call `addTrustee`/`removeTrustee(atDepth: currentDepth)` directly per toggle, the same way `Group+FormV3` already works — not "batch the whole selection and merge on save." `ShardCustodyManager.saveGlobalShardConfig(mergingVisibleSelection:isVisible:)` becomes dead code once this lands, and `VaultGlobalTrustees.swift`'s save flow needs reworking, not just the data model underneath it.

**Migration: lazy-on-first-touch**, not eager-at-launch. `Group` migrates `deeperMemberSlots` lazily specifically because there can be many groups, and an eager sweep across all of them is what caused real device crashes (reverted). `GlobalShardConfig` is a singleton — exactly one row, ever — so that specific risk doesn't apply; migrating it eagerly would cost one key derivation regardless of when it happens. Lazy-on-first-touch is still the right call anyway, for a simpler reason: it avoids adding a new app-launch migration hook as a separate integration point. A guard inside `addTrustee`/`removeTrustee`/`trustees(atDepth:)` — "if slots aren't populated yet, migrate from the old flat field first" — is less surface area than a launch-time routine.

**The overage case: existing configs may already exceed 16, and can't be silently truncated.** There's never been a cap before this fix, so it's entirely possible some existing installation already has more than 16 (or more than whatever cap is chosen) trustees selected. Two approaches considered and rejected:
- *Silently truncate to the first 16* — silent, unannounced weakening of someone's actual recovery setup. Unacceptable for a security-relevant setting.
- *Let depth 0's array stay oversized for grandfathered data, cap only new selections and duress depths* — reintroduces exactly the leak fixed-size padding exists to prevent: if depth 0's stored array is a different length than every other depth's, an examiner can tell "this is the real depth" from ciphertext size alone, without decrypting anything.

**Resolved approach:** detect the overage explicitly and don't touch the data until the user acts. If `ShardCustodyManager` finds an existing flat `trusteeIDs` count exceeding 16, don't migrate yet — leave the old flat format in place, fully functional for its current purpose (pre-populating `VaultShardSetup`'s suggestions), nothing breaks in the interim. `VaultGlobalTrustees` shows the full existing list with a clear message ("N trustees selected; the new limit is 16 — deselect down to continue") and blocks saving until the user is at or under the cap. Only once they're within bounds does migration into padded per-depth slots actually happen. This means `ShardCustodyManager`'s read path needs to tolerate three states: not-yet-migrated-and-fits (auto-migrates transparently), not-yet-migrated-and-over-cap (blocks until pruned), and migrated (normal per-depth operation). This handling is needed regardless of which slot count is chosen — any finite cap could in principle already be exceeded by existing unbounded data.
...
Items 1 and 2 (filtering + the merge-not-overwrite save fix) are independently useful and lower-risk — they close the live leak even with zero decoy content, and don't depend on anything else here. **Done** (`9e439e3`). Items 3, 4, and 5 (the actual decoy machinery) are a materially bigger undertaking, only worth it once 1 and 2 are done and confirmed safe on their own — **item 3 is now fully scoped with all decisions made (see above), not yet implemented.** Note item 3 isn't purely additive on top of 1–2: it makes `VaultGlobalTrustees.swift`'s already-shipped merge-not-overwrite save path obsolete, reworking it rather than building alongside it.
