...
**Status:** in progress, the rejection-based oracle is now closed. Final design settled 2026-08-08 (see "Final design: `passSecurityControl` removed entirely" near the bottom). **Implemented (`d0d75b5`, `314cf18`, `b1f9045`):** `originDepth` (floor semantics, the display-visibility half), the `handleReplace` ownership-check fix it depended on, and `passSecurityControl` itself removed — no bundle is ever rejected for restriction state anymore, for any contact, real or duress-origin, existing or newly-paired. The active-tester variant (duress-origin or existing-contact) closes with zero residual risk, since the coercer is always the author of what gets decrypted back to them. What remains is narrower and different in kind: a deliberately accepted, timing-bounded **content-confidentiality** risk for the passive variant only (a real, unaware third party's message rendering on screen during a coercion window) — not a detection oracle, since there's no longer any sender-dependent rejection signal to detect. **Not yet implemented:** C2's `onDuress` still unconditionally discards queued files rather than processing them — the one remaining place restriction still produces different behavior from normal unlock. A 2026-08-07 pass verified the "wording parity" open question below is already satisfied by the shipped code and closed one adjacent gap found while verifying it — see "2026-08-07 — Wording parity verified..." near the bottom. Found while scoping shard-custody items 4–5 (`Shard-Custody-Not-Cleaned-Up-On-Contact-Deletion.md`) — surfaced when checking whether a coercer-controlled second device sending a real shard could be used to detect duress mode.
