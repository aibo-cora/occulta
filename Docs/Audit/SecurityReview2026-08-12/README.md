...
- **Status:** ⚠️ **FIXED 2026-08-12 — Bug 80 — but the fix regressed backward compatibility; see Bug 81.** A contact genuinely on a pre-1.10.0 build whose marker was stranded is now blocked permanently rather than until they update, because neither healing path survives the fix. The gate now fails closed on a stranded marker, and the capability tier it reads is a high-water mark so it cannot be lowered by a bundle claiming an old build. The lost version values remain unrecoverable; what that costs is an interop trade, not the bypass. One operational item (how affected users are told) is still open.
