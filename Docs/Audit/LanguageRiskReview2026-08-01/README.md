...
**Addendum (2026-08-09) — a second class of language risk exists, and this review does not look for it.** Everything above concerns wording that reads as *intent to defeat a lawful process*. The inverse failure is **overclaiming a protective guarantee to users who will rely on it**: copy that promises a security or anti-fraud property the design cannot actually deliver. Exposure there is consumer-protection/misrepresentation, not obstruction, and it does not track the same signals — it is worst in the features with the most mainstream audience and the least security-literate users, which is the opposite of where the Tier 2–4 findings cluster.

Raised while reviewing Verified Payment Cards, whose audience (home buyers, adults protecting elderly parents) is the least equipped to evaluate a cryptographic claim and whose per-incident loss is the largest on the roadmap. That feature's own doc now carries explicit copy rules and a set of banned formulations — see [Payment Cards/FINDINGS.md](../../Features/Payment%20Cards/FINDINGS.md), "Positioning and copy discipline" — with the governing test being that **every claim must be about what the app displays, never about where money goes**.

A future pass should audit for both classes. The same routing recommendation applies to each: through counsel before publication.
