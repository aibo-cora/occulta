...
## Moved out — the payments application
...
- **`#26` can deliver its core anti-BEC value without `#15`.** The signed request replaces `#26` rule (3)'s pairing with a live presence check, removing the dependency that currently drags `#26` into this delay.
- **Q-01's residual is narrower than stated above.** A pinned destination means that even a fully deceived victim's funds can only reach the counterparty's real account, so the policy a family must hold becomes "money only goes to a card you already have" — a fixed constraint rather than a judgement call under pressure. The behavioural dependency does not disappear; it gets smaller and more concrete. See Payment Cards D-05.

**Neither unblocks `#15`.** SPEC.md's construction and its §6 delay stand exactly as written.
