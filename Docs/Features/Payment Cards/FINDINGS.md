...
**Ruling 2026-08-14:** build, but **v0 only, one wedge, and not before gate zero** — see [Viability ruling](#viability-ruling--2026-08-14) and the v0/deferred split in [Action items](#action-items). **Amended the same day: the crypto lead is withdrawn** and v0 is held rather than re-scoped, because the doc had costed the feature against products and never against the free practices people actually use (D-18). Shelving is a live outcome. **Two further findings, same day:** the protection is intermittent rather than continuous, and the gap is when the attack is easiest (D-19); and iOS-only does not shrink the leading wedge's market so much as break its mechanism, since that wedge's protection is a behavioural policy (D-20). Both bear on gate zero; neither changes the ruling.
...
- **The device-replacement window.** After either side replaces a device, a legitimate re-issue and an attacker's *"I got a new phone, here's my new card"* are indistinguishable — both unsigned by the key on file, both rendering unverified, with Q-07's invalidation firing identically for each. Recurs roughly every two to three years per relationship, per side, and is the interval where the attacker's move is the one the user already expects. See D-19.
...
### D-19 · The protection is intermittent, and the gap is when the attack is easiest

*Added 2026-08-14. Sharpens point 2 of the [viability ruling](#viability-ruling--2026-08-14), which prices device-replacement decay as a cost without noting when it is exploitable.*

A card protects through two mechanisms: the contact-wide diff (a change to a known destination is loud) and relationship-relative age (D-06). **A device replacement on either side resets both**, because the new device holds a new key and every prior signature was made by the old one.

The finding is what that reset looks like from inside the app. A legitimate *"I replaced my phone, here is my new card"* and an attacker sending exactly that sentence are **indistinguishable** — neither is signed by the key on file, both render unverified, and no cryptographic test separates them, because in the legitimate case the key genuinely did change. Q-07's loud invalidation is correct and necessary, and it is **not** a distinguisher: it fires identically in both cases, by construction.

So the protection is not continuously available. It has a recurring hole — roughly every two to three years per relationship, per side — and inside that hole the attacker's cheapest move is the sentence the user is already expecting from a legitimate counterparty. The feature does not fail open there; it fails **undecided**, and undecided is the state users resolve by doing the convenient thing.

The resolution is another physical meeting, which the viability ruling already prices. What it did not say is that the window where the meeting has not happened yet is the window where the attack is easiest, not merely where the cost is paid.

**Consequence.** The value claim is *intermittent* protection, not continuous. Nothing in copy, UI, or internal expectation may imply a card protects a relationship continuously from exchange onward — it protects the intervals between device replacements. Carry this into [Positioning](#positioning-and-copy-discipline) alongside the existing reliance rule.

### D-20 · Single platform does not shrink the market, it breaks the mechanism

*Added 2026-08-14. Recorded because `Android` appeared **zero times** in this document before this finding, and the omission is load-bearing for the one wedge that carries the value.*

For the consumer wedges, iOS-only is a quantity problem: a smaller pool, priced the same way any addressable-market limit is priced.

For **small business ↔ recurring vendor** — the wedge the [ruling amendment](#amendment--the-lead-wedge-is-withdrawn) identifies as where the residual actually lives — it is a different kind of problem, because the security mechanism there **is a policy**. D-07 already records the ceiling: *"only pay to an existing card"* is behavioural, not cryptographic. A policy is only as strong as its exception list, and a mixed-platform vendor roster forces a standing exception into it: *"just send it the normal way, I'm on Android."*

Once that sentence is routine, an attacker says it too — and it is indistinguishable from the legitimate version, the same shape as D-19. The exception is not a gap in coverage the policy survives; it is a documented, socially normal bypass that the payer has been trained to accept.

So the wedge with the highest value-add is the one most dependent on an exception-free policy, and the one structurally least able to have one. That is a worse problem than two-sided cold start, because cold start is a growth curve and this is a ceiling.

**Not fixable inside this feature.** Android support is a product-level decision well outside payment cards, and nothing in this design changes if it never happens.

**Consequence for gate zero.** Question 0a must be put to a business with a realistic vendor roster, not a hypothetical one. A positive answer from a counterparty set that happens to be all-iPhone does not generalise, and would be the same error as the withdrawn crypto lead — a wedge ranked on the axis that was easiest to measure.
...
**2 · Cards decay, and the decay is not a user segment.** *"Anyone whose counterparty changes phones"* sits in the exclusions above as though it named a cohort to write off. It does not. It is a decay function across the whole installed base, firing once per relationship every time either side replaces a device — on the order of every two to three years, for every pair. Real estate absorbs it, because a transaction closes in weeks. Small business and family are the two cases whose value *comes from* accrued age (D-06), and they are exactly the ones the reset lands on. Q-07's loud invalidation is the correct handling and does not change the economics: every reset costs another physical meeting. **Sharpened the same day by D-19:** the interval before that meeting happens is not merely when the cost is paid, it is when the attack is easiest — a legitimate re-issue and *"I got a new phone, here's my new card"* are indistinguishable, so the protection is intermittent rather than continuous.
...
**Constrained the same day by D-20, before anyone promotes it.** That wedge's mechanism is a behavioural policy, and this app is iOS-only — so a mixed-platform vendor roster forces a standing *"send it the normal way, I'm on Android"* exception that an attacker can use verbatim. The wedge with the highest value-add is the one least able to hold an exception-free policy. Promoting it on the strength of the residual alone would repeat the withdrawn crypto lead's error in the other direction.
...
**0a · Is the residual worth paying for?** D-18 establishes that the in-person QR scan, the $1 test transaction and a phone call are free, standard, and already cover first entry. What cards add is a *later* change made loud. So: would a small business pay for that instead of phoning the vendor? This question comes first, because a negative answer makes 0b irrelevant — and neither question needs a line of code. **Condition on who is asked (D-20):** the business must have a realistic, mixed-platform vendor roster. A positive answer from one whose counterparties all happen to carry iPhones proves nothing and would repeat the withdrawn crypto lead's error — ranking a wedge on the axis that was easiest to measure. **And state the offer honestly when asking (D-19):** what is on sale is protection between device replacements, not from exchange onward.
...
| D-19 | 2026-08-14 — the device-replacement reset was priced as a cost by the viability ruling but never located in time. A legitimate re-issue and an attacker's *"I got a new phone"* are indistinguishable, Q-07's invalidation fires identically for both, and the interval before the replacement meeting is when the attack is easiest. Yields an intermittent rather than continuous protection claim, and a new Residual entry |
| D-20 | 2026-08-14 — `Android` occurred zero times in this document. For consumer wedges iOS-only is a market limit; for small business ↔ vendor it attacks the mechanism, since that wedge's protection is a behavioural policy (D-07) and a mixed-platform roster forces an exception an attacker can use verbatim. Constrains the amendment's promotion of that wedge and adds a realistic-roster condition to gate-zero question 0a |
