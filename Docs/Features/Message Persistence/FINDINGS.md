...
**Correction (found while re-checking D-05 elsewhere in this file): the analogy below assumed `activateSecureMode` currently relocates sensitive contacts into `LayerStore` for genuine cryptographic isolation. It doesn't — confirmed, contacts are never removed from the main store; the `LayerStore` blob is a restoration snapshot for deactivation, not a confidentiality mechanism, and every contact stays re-encrypted under the same canonical key regardless of classification. So this option isn't "extend an existing, working mechanism to messages" — it's "build genuine structural isolation from scratch," since nothing in this codebase does that today, contacts included.** That makes it a bigger, not smaller, undertaking than originally framed here — reinforcing, not weakening, why it's out of scope for this pass.

Original framing, for the record: analogous to `activateSecureMode`'s `LayerStore` relocation for sensitive contacts — each contact's messages would live in a separate encrypted container, keyed by something only derivable if you already trust that specific contact, with no distinguishable row in the main store when hidden. This is the only option that closes the gap completely rather than raising the effort bar. `LayerStore`'s fixed-size-slot trick (§B3 — constant file size regardless of payload) is real and does exist, independent of the correction above — but it's a padding/uniformity trick applied to a restoration snapshot, not evidence of a working relocation-for-confidentiality precedent to extend. Messages are also unbounded and continuously growing, so even a genuine version of this needs pagination/archival solved on top before it generalizes. Confirmed as its own initiative, matching Q-03 — not a Sweep 2 sub-task.
...
**Correction: the protection benefit below doesn't actually exist, per the D-05 correction elsewhere in this file.** `Contact.Profile` gets no confidentiality benefit from being classified sensitive — it stays encrypted under the same canonical key as every other contact, always. Folding a message into that payload would inherit that same (non-)protection, not something stronger. This option is weaker than originally framed, not just expensive.

Original framing, for the record: the appeal was that a message folded into the already-hidden `Contact.Profile` payload would automatically inherit whatever protection that row already has, and D-06's whole class of gap would disappear by construction. Cost, independent of the correction above: AES-GCM seals a blob as a whole, not incrementally, so every new message means decrypt-entire-history, append, re-encrypt, rewrite the whole thing back — unboundedly more expensive as a conversation grows, the wrong shape for the workload that actually dominates (sending/receiving happens far more often than opening an old thread cold). Rejected on cost alone even before the correction above; the correction just removes the one reason it might otherwise have been worth that cost.
...
Even with Q-02 solved and every `Contact.Message` row correctly Tier-1-tagged, rows still physically exist in the database regardless of depth — true structural hiding is a separate, larger security initiative. **Correction: it does not "today only cover full Secure Mode contact activation" as originally stated here — per the D-05 correction, no true structural hiding exists anywhere in this codebase today, contacts included.** This residual limitation is bigger than originally scoped: it's not "messages haven't caught up to contacts yet," it's "true structural hiding doesn't exist for anything yet." Still named as a known, explicit residual limitation rather than silently left unaddressed or accidentally implied as solved by Sweep 2 — that part of the original framing stands.
...
Sequenced right after (or alongside) contact classification — it needs to know which contacts are being classified sensitive-at-this-layer (and pushed into the `LayerStore` restoration snapshot alongside staying in the main store, re-encrypted like everyone else — see the D-05 correction above) in *this* activation pass:
...
This dominates delete-based purging on the existence axis specifically. **A composability claim originally made here — that it would fold in cleanly with per-contact-key content sealing, since real or Tier 2-relocated protection would then apply automatically to whichever content currently occupies the row — no longer holds, per the D-05 correction: there is no Tier 2-relocated protection for it to inherit.** Per-contact-key sealing was decided against below on its own, narrower grounds (the purge-invocation gap it would have hedged against is now closed; its staleness benefit only ever covered content, not `encryptedRecipientID`) — this correction removes what would have been the strongest reason to reconsider that decision, rather than changing it.
...
## Design note: two coexisting duress strategies, not a contradiction

This document's chaff design and the rest of the app's Secure Mode mechanism pursue opposite strategies for the same underlying problem — what happens to sensitive content at a duress boundary. Worth stating explicitly in one place, since reading either in isolation could look like it contradicts the other.

**Strategy 1 — hard destruction, openly.** Secure Mode activation rotates the canonical local DB key and deletes the old one (§S1, cited above in "Key"); the Vault's dedicated tier layers a fresh biometric SE-key evaluation on top (§S8, Q-04). Content sealed under the old key becomes permanently unreadable the instant activation completes — not hidden, not filtered, gone. The triggering event (activation, or a duress-PIN entry per "New hook" above) is itself ordinary observable app behavior, not concealed — depth changes, a layer gets created. This is the same posture GrapheneOS's duress wipe takes: reliable, irreversible key destruction over any attempt to make the destruction itself invisible, on the reasoning that a sophisticated adversary who already knows the mechanism exists sees through concealment anyway, and concealment machinery is itself extra attack surface.

**Strategy 2 — plausible deniability, at the content layer.** Chaff-backed drafts do the opposite: every row exists at every depth, always, so whether a row was ever real is never an observable event. This exists because drafts have no Strategy-1-shaped option available to them — D-05's correction established that nothing in this codebase, contacts included, has genuine structural key isolation per classification tier; everything sensitive or not stays in one queryable table under the same canonical key at rest. Absent a real Tier 2 to fall back on, the only way to defeat a *bulk/automated* existence-tell is to stop appearance and disappearance from being events at all.

**Why both, without contradiction.** They answer different questions. Strategy 1 answers "can this content ever be read again by someone who doesn't hold the new key" — destroyed, unconditionally, and it's fine that the destruction event is visible, because the alternative (a live, readable key) is worse regardless of who's watching. Strategy 2 answers "can the mere existence of a row, independent of its content, be used to infer a hidden contact" — and where that's the live threat, concealing the event matters more than destroying content Strategy 1 was never reachable for in the first place, for lack of a tier to destroy it into. Chaff never substitutes for key destruction where key destruction is available: drafts still ride the same canonical-key rotation as everything else at activation (see "Purge on Secure Mode activation" above). Chaff only covers the layer key rotation was never going to solve.

**The honest limit, restated from "The honest ceiling" above.** Chaff is explicitly scoped to defeat bulk/statistical inspection, not a resourced adversary doing manual linguistic forensics against a reverse-engineered generation scheme — the same ceiling GrapheneOS accepts for its own wipe (no claim to beat a maximally resourced, target-specific adversary, only to make the common case reliably safe). Neither strategy is chosen for being theoretically perfect; each is the strongest available answer to the specific threat shape it targets, and the two don't compete for the same threat.
