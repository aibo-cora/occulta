...
struct OccultaApp: App {
    ...
    private struct SecuritySetting: View {
        ...
        var body: some View {
            ProgressView()
                .task {
                    self.sceneDelegate.security = self.security
                    self.signal.toggle()
                }
        }
    }
    
    @State private var securitySettingRunning: Bool = true

    var body: some Scene {
        WindowGroup {
            if self.securitySettingRunning {
                SecuritySetting(signal: self.$securitySettingRunning)
