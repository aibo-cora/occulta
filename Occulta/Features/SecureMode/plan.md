...
- [x] **[security]** Share index management — ownership consolidated to authentication events, not scene phase handlers. (Bugs 65–68)
  ...
  **Open (Bug 69):** cold launch with `pinEnabled = false, isSecureModeActive = true` — `onAuthenticated` never fires, `shareIndexAllowedIDs` stays `nil` in memory, first contact mutation overwrites the share index with all contacts. Fix: initialise `shareIndexAllowedIDs` inside `AppScreen.evaluate(coldLaunch:)` on the no-PIN-gate path. Pending.
...
- **Share index uninitialised on cold launch with gate down. Open (Bug 69).** When `pinEnabled = false, isSecureModeActive = true`, no authentication event fires on launch. `shareIndexAllowedIDs` stays `nil`; the first contact mutation overwrites the share index with all contacts, exposing contacts hidden from the depth-1 view. Fix: initialise `shareIndexAllowedIDs` in the no-PIN-gate path of `AppScreen.evaluate(coldLaunch:)`. Pending implementation.
